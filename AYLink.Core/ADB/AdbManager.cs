...
public sealed class AdbManager
{
    ...
    public async void DisconnectDevice(string serial)
    {
        if (_connectedDevices.TryGetValue(serial, out _))
        {
            try
            {
                await _adbClient.DisconnectAsync(serial);
                _connectedDevices.Remove(serial); // 从字典中移除
                Debug.WriteLine($"设备 {serial} 已断开");
            }
            catch
            {
                Debug.WriteLine($"设备 {serial} 无法断开");
            }
        }
    }
