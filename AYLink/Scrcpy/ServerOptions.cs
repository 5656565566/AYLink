...
public class ServerOptions
{
    ...
    /// <summary>
    /// 指定服务端要捕获的显示器ID使用 list_displays=true 获取可用ID
    /// </summary>
    ...
    /// <summary>
    /// 将所有设置转换为 scrcpy-server.jar 可接受的参数列表
    /// </summary>
    public List<string> ToArgs()
    {
        ConfigManager _configManager = ConfigManager.Instance;
        Config appConfig = _configManager.LoadConfig<Config>("appConfig");
        string? scrcpyVersion = string.IsNullOrWhiteSpace(appConfig.ScrcpyVersion)
            ? null
            : appConfig.ScrcpyVersion;

        if (scrcpyVersion != null) {
            ClientVersion = scrcpyVersion;
        }
