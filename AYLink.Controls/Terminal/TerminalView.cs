...
namespace AYLink.Controls.Terminal
{
    ...
    public class TerminalView : Control
    {
        ...
        // Process management (used for cleanup/cancellation)
        ...
        /// <summary>
        /// Event raised when user input is received (keystrokes, paste, etc.) that should be
        /// forwarded to the connected device (e.g., ADB shell socket).
        /// </summary>
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        protected override async void OnKeyDown(KeyEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    e.Handled = true;
                    RaiseUserInput(printableChar.ToString());
                    return;
                }
        ...
        protected override void OnPointerWheelChanged(PointerWheelEventArgs e)
        {
            ...
            if (_terminal.MouseTrackingMode != XT.Input.MouseTrackingMode.None)
            {
                var point = e.GetPosition(this);
                var col = (int)(point.X / _charWidth);
                var row = (int)(point.Y / _charHeight);
            ...
            if (delta != 0)
            {
                // If we are in alternate buffer (like top/vim), send arrow keys instead of scrolling
                if (this.IsAlternateBuffer)
                {
                    // Send Up/Down arrow keys based on scroll direction
                    string sequence = delta > 0 ? "\x1b[A" : "\x1b[B";
                    // Send multiple times based on scrollLines
        ...
        private void OnTerminalDataReceived(object? sender, XT.Events.TerminalEvents.DataEventArgs e)
        {
            ...
            // Forward to external handler via UserInput event
            RaiseUserInput(e.Data);
        }

        /// <summary>
        /// Raises the UserInput event to notify external subscribers (e.g., ADB shell connector)
        /// that the user has typed input that should be forwarded to the connected device.
        /// This replaces the original SendToPtyAsync method.
        /// </summary>
        private void RaiseUserInput(string data)
        {
        ...
        /// <summary>
        /// Writes data to the terminal emulator for rendering.
        /// Call this method to display output received from the connected device (e.g., ADB shell output).
        /// The data is processed by XTerm.NET which handles VT escape sequences, colors, cursor movement, etc.
        /// </summary>
        public void WriteToTerminal(string data)
        {
            if (string.IsNullOrEmpty(data) || _terminal == null)
                return;
            ...
            Dispatcher.UIThread.Post(() =>
            {
                ...
                // Alternate buffer (used by full-screen apps like vim, htop)
                ...
                if (!_isAlternateBuffer)
                {
                    ...
                    if (y != _terminal.Buffer.ViewportY)
                        RaisePropertyChanged(ViewportYProperty, y, _terminal.Buffer.ViewportY);
                }
        ...
        /// <summary>
        /// No-op: PTY process launching is not used in this terminal.
        /// Data input/output is handled externally via WriteToTerminal() and UserInput event.
        /// </summary>
        private void LaunchProcess()
        {
            // No-op: external I/O via WriteToTerminal() and UserInput event
        }
        ...
        private void RenderNormalLine(DrawingContext context, BufferLine line, int screenY, double startYPos, double rowHeight, double scale)
        {
            ...
            if (textRuns != null)
            {
                foreach (var run in textRuns)
                {
                    ...
                    if (run.Background != null && run.Background.Opacity > 0 && run.Background != Brushes.Transparent)
                    {
            ...
            for (int x = 0; x < _terminal.Cols;)
            {
                ...
                if (background != null && background.Opacity > 0 && background != Brushes.Transparent)
                {
                    ...
                    if (IsFocused)
                    {
                        ...
                        if (line != null && cursorX < line.Length)
                        {
                            ...
                            var formattedText = new FormattedText(
                                ...
                                typeface,
                                FontSize,
                                invertedBrush == Brushes.Transparent ? this.Foreground : invertedBrush);
